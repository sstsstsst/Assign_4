...
using namespace std; 
  ...
  void writeToFile(char* fileLoc, const char* str);
...
execClient(int argc, char **argv) {
  ...
  char* logFileLoc = argv[4];


  clearFile(logFileLoc);
  ...
  char sendline[MAXLINE], recvline[MAXLINE];
   ...
   //client connected
   std::string logStr = getTimestamp() + "\tclient\tpid is [" +  std::to_string(getpid()) + "]\t\t :\tconnected.\n";
   writeToFile(logFileLoc, logStr.c_str());
    ...
    //line is a line in the commandFile
      ...
      //sent to Server
        ...
        for (int i = 0; i < MAXLINE; i++){
          ...
          logStr = getTimestamp() + "\tclient\tpid : [" +  std::to_string(getpid()) + "]\t\t sending:\t" + sendline + '\n';
          writeToFile(logFileLoc, logStr.c_str());
        ...
        if (recv(sockfd, recvline, MAXLINE,0) == 0){
          ...
          logStr = getTimestamp() + "\tclient\tpid : [" + std::to_string(getpid()) + "]\t\t recieved:\t" + recvline + '\n';
          writeToFile(logFileLoc, logStr.c_str());
