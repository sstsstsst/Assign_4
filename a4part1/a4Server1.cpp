...
int execServer(int argc, char **argv){
  ...
  cout << getTimestamp() << "\tpid is [" << getpid() << "]\tport is (" << SERV_PORT << ")" << "\t...waiting for connections." << endl;
  ...
  // printf("%s\n","Received request...");
   ...
   if ( (childpid = fork ()) == 0 ) { //if it’s 0, it’s child process
      cout << getTimestamp() << "\tpid is [" << getpid() << "]\tChild created for dealing with client requests" << endl;
